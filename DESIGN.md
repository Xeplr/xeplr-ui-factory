...
| Records | **Real tables, never JSON.** One table per entity, one column per field, named as the field. **Publish applies the change directly** (no migration files): create, add, widen; narrowing / type changes refused; a removed field **drops its column after confirmation** — never a hand-made column, never one another tenant's screen still uses. Names locked once they are columns. |
